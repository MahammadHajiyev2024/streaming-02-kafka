...
Kafka consumer: USD-only filter example.

Reads sales messages from a Kafka topic.
Processes only USD transactions, skipping other currencies.
...
LOG = get_logger("H02", level="DEBUG")
...
OUTPUT_CSV: Final[Path] = OUTPUT_DIR / "consumed_sales_hajiyev_usd.csv"
...
def process_message(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if row.get("currency_code") != "USD":
        LOG.info(f"SKIPPED: Non-USD currency ({row.get('currency_code')})")
        return None
...
def log_summary(consumed_count: int, settings: KafkaSettings) -> None:
    ...
    LOG.info("Summary:")
    LOG.info(
        f"Consumed and saved {consumed_count} USD message(s) from topic {settings.topic!r}."
    )
    log_path(LOG, "OUTPUT_CSV", OUTPUT_CSV)
    LOG.info("========================")
